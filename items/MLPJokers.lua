...
SMODS.Joker { -- Applejack
    ...
    calculate = function(self, card, context)
        if context.cardarea == G.play and context.repetition and not context.repetition_only then
            ...
            if context.other_card == context.scoring_hand[#context.scoring_hand] or context.other_card ==
                context.scoring_hand[#context.scoring_hand - 1] then
                return {
...
SMODS.Joker { -- Spike
    ...
    calculate = function(self, card, context)
        if context.cardarea == G.jokers and context.before then
            if context.scoring_name == card.ability.extra.poker_hand and #G.consumeables.cards +
        ...
        if context.joker_main then
            ...
            for i = 1, #context.scoring_hand do
                ...
                elseif SMODS.has_any_suit(context.scoring_hand[i]) then
                    ...
                    table.insert(playedcards, context.scoring_hand[i].base.id .. context.scoring_hand[i].base.suit ..
                        context.scoring_hand[i].ability.name)
                end
                -- print(playedcards)
            end
...
SMODS.Joker { -- Trixie
    ...
    loc_vars = function(self, info_queue, card)
        return {
            vars = {card.ability.extra.cards_to_destroy}
        }
    end,
    calculate = function(self, card, context)
...
SMODS.Joker { -- Sunset Shimmer
    ...
    calculate = function(self, card, context)
        ...
        if context.before and context.main_eval and G.GAME.current_round.hands_played == 0 and not converted and
...
SMODS.Joker { -- Lyra and Bon Bon
    ...
    loc_vars = function(self, info_queue, card)
        return {
            vars = {card.ability.extra.hand_size, card.ability.extra.discards}
        }
    end,
...
SMODS.Joker { -- Apple Bloom
    ...
    calculate = function(self, card, context)
        if context.individual and context.cardarea == G.play then
            if #context.scoring_hand > 1 and context.scoring_hand[#context.scoring_hand]:get_id() ==
                context.scoring_hand[#context.scoring_hand - 1]:get_id() then
                if context.other_card == context.scoring_hand[#context.scoring_hand] or context.other_card ==
                    context.scoring_hand[#context.scoring_hand - 1] then
                    return {
...
SMODS.Joker { -- Scootaloo
    ...
    loc_vars = function(self, info_queue, card)
        return {
            vars = {card.ability.extra.dollars, card.ability.extra.dollars_gain}
        }
    end,
    calculate = function(self, card, context)
        if context.end_of_round and G.GAME.current_round.hands_played == 1 and not context.repetition and
...
SMODS.Joker { -- Big McIntosh
    ...
    loc_vars = function(self, info_queue, card)
        return {
            vars = {card.ability.extra.retriggers, card.ability.extra.odds, G.GAME.probabilities.normal}
        }
    end,
    rarity = 2,
    atlas = 'MLPJokers',
...
SMODS.Joker { -- Zecora
    ...
    loc_vars = function(self, info_queue, card)
        return {
            vars = {card.ability.extra.odds, G.GAME.probabilities.normal}
        }
    end,
    rarity = 1,
    atlas = 'MLPJokers',
    ...
    calculate = function(self, card, context)
        if context.before and context.main_eval and not context.blueprint and #context.full_hand == 4 then
            if pseudorandom(pseudoseed('potionbrewing')) < G.GAME.probabilities.normal / card.ability.extra.odds and
...
SMODS.Joker { -- Derpy
    ...
    calculate = function(self, card, context)
        if context.individual and context.cardarea == G.play and context.other_card.lucky_trigger and
            not context.blueprint then
            return {
...
SMODS.Joker { -- DJ PON-3
    ...
    loc_vars = function(self, info_queue, card)
        return {
            vars = {card.ability.extra.chips, G.GAME.current_round.MLPMUSIC_card.rank}
        }
    end,
    rarity = 2,
    atlas = 'MLPJokers',
...
SMODS.Joker { -- Octavia Melody
    ...
    loc_vars = function(self, info_queue, card)
        return {
            vars = {card.ability.extra.mult, G.GAME.current_round.MLPMUSIC_card.rank}
        }
    end,
    rarity = 2,
    atlas = 'MLPJokers',
...
function Game:init_game_object()
    local ret = igo(self)
    ret.current_round.MLPMUSIC_card = {
        id = 14,
        rank = 'Ace'
    }
    return ret
...
function SMODS.current_mod.reset_game_globals(run_start)
    if run_start then
        ...
        if G.GAME.current_round.MLPMUSIC_card.id == 14 then
            G.GAME.current_round.MLPMUSIC_card.rank = 'Ace'
        elseif G.GAME.current_round.MLPMUSIC_card.id == 13 then
            G.GAME.current_round.MLPMUSIC_card.rank = 'King'
        elseif G.GAME.current_round.MLPMUSIC_card.id == 12 then
            G.GAME.current_round.MLPMUSIC_card.rank = 'Queen'
        elseif G.GAME.current_round.MLPMUSIC_card.id == 11 then
            G.GAME.current_round.MLPMUSIC_card.rank = 'Jack'
        else
            G.GAME.current_round.MLPMUSIC_card.rank = G.GAME.current_round.MLPMUSIC_card.id
        end
    end
end
...
SMODS.Joker { -- Discord
    ...
    loc_vars = function(self, info_queue, card)
        return {
            vars = {card.ability.extra.xmult, localize(card.ability.extra.poker_hand, 'poker_hands')}
        }
    end,
    set_ability = function(self, card, initial, delay_sprites)
        local _poker_hands = {}
        ...
        card.ability.extra.xmult = (pseudorandom("klutzydraconequus", card.ability.extra.xmultmin,
            card.ability.extra.xmultmax) / 10)
    end,
    calculate = function(self, card, context)
        if context.discard or context.final_scoring_step and not context.blueprint then
            ...
            card.ability.extra.poker_hand =
                pseudorandom_element(_poker_hands, pseudoseed("whatfunisthereinmakingsense"))
            card.ability.extra.xmult = (pseudorandom("klutzydraconequus", card.ability.extra.xmultmin,
                card.ability.extra.xmultmax) / 10)
        end
...
SMODS.Joker { -- Elements of Harmony
    ...
    loc_vars = function(self, info_queue, card)
        return {
            vars = {card.ability.extra.xmult, card.ability.extra.jokers}
        }
    end,
    calculate = function(self, card, context)
        if context.joker_main then
...
SMODS.Joker { -- Dragon Code
    ...
    loc_vars = function(self, info_queue, card)
        return {
            vars = {card.ability.extra.tarotmake}
        }
    end,
    calculate = function(self, card, context)

        if context.using_consumeable and context.consumeable.ability.set == "Tarot" and #G.consumeables.cards +
            G.GAME.consumeable_buffer < G.consumeables.config.card_limit and not card.ability.extra.tarotmake then
...
SMODS.Joker { -- Library Card
    ...
    loc_vars = function(self, info_queue, card)
        return {
            vars = {card.ability.extra.mult, card.ability.extra.mult_gain, card.ability.extra.lasthandtype}
        }
    end,
    calculate = function(self, card, context)
        if context.before and not context.blueprint then
...
SMODS.Joker { -- Mural of Competition (L)
    ...
    calculate = function(self, card, context)
        if context.cardarea == G.play and context.repetition and not context.repetition_only then
            if (context.other_card:get_id() == 6 or context.other_card:get_id() == 7 or context.other_card:get_id() == 8 or
                context.other_card:get_id() == 9 or context.other_card:get_id() == 10) then
                return {
                    message = localize('k_again_ex'),
                    repetitions = card.ability.extra.repetitions,
                    card = card
...
SMODS.Joker { -- Sunburst
    ...
    blueprint_compat = true,
    calculate = function(self, card, context)
        if context.individual and context.cardarea == G.play and #G.consumeables.cards + G.GAME.consumeable_buffer <
            ...
            if context.other_card.seal == 'Blue' or context.other_card.seal == 'Purple' then
                ...
                if powerup < 0.02 then
                    consumeabletype = 'Spectral'
                elseif powerup > 0.65 then
                    consumeabletype = 'Planet'
                else
                    consumeabletype = 'Tarot'
                end
                G.GAME.consumeable_buffer = G.GAME.consumeable_buffer + 1
                return {
                    extra = {
...
SMODS.Joker { -- Gilda
    ...
    loc_vars = function(self, info_queue, card)
        return {
            vars = {card.ability.extra.mult, card.ability.extra.mult_mod, card.ability.extra.money_req,
    ...
    calculate = function(self, card, context)
        ...
        if context.MLP_ease_dollars and to_big(context.MLP_ease_dollars) < to_big(0) and not context.blueprint then
            card.ability.extra.money_spent = to_big(card.ability.extra.money_spent) - context.MLP_ease_dollars
...
SMODS.Joker { -- Gabby
    ...
    calculate = function(self, card, context)
        if context.post_trigger and context.other_card ~= card and context.other_ret.jokers then
            if (context.other_ret.jokers.chips or context.other_ret.jokers.mult or context.other_ret.jokers.h_chips or
                context.other_ret.jokers.h_mult or context.other_ret.jokers.chip_mod or
                context.other_ret.jokers.mult_mod) then
                card.ability.extra.chips = card.ability.extra.chips + card.ability.extra.chips_gain
                return {
                    focus = card,
                    message = localize('k_upgrade_ex'),
...
SMODS.Joker { -- Cheerilee
    ...
    loc_vars = function(self, info_queue, card)
        return {
            vars = {card.ability.extra.chipmult}
        }
    end,
    calculate = function(self, card, context)
        if context.individual and context.cardarea == G.play and context.other_card then
            if (context.other_card:get_id() == 2 or context.other_card:get_id() == 3 or context.other_card:get_id() == 4 or
...
SMODS.Joker { -- Spitfire
    ...
    loc_vars = function(self, info_queue, card)
        return {
            vars = {card.ability.extra.mult, card.ability.extra.odds, G.GAME.probabilities.normal}
        }
    end,
    calculate = function(self, card, context)
        if context.individual and context.cardarea == G.play then
...
SMODS.Joker { -- Dr. Hooves
    ...
    loc_vars = function(self, info_queue, card)
        return {
            vars = {card.ability.extra.mult, card.ability.extra.mult_gain, card.ability.extra.odds,
                    G.GAME.probabilities.normal}
        }
    end,
    calculate = function(self, card, context)
        if context.end_of_round and
            (pseudorandom(pseudoseed('allonsy')) < G.GAME.probabilities.normal / card.ability.extra.odds) and
            not context.repetition and context.game_over == false and not context.blueprint then
            card.ability.extra.mult = card.ability.extra.mult + card.ability.extra.mult_gain
            return {
                message = localize('k_upgrade_ex'),
                colour = G.C.MULT,
    ...
    calculate = function(self, card, context)
        if context.final_scoring_step and not context.blueprint then
            ...
            for handname, values in pairs(G.GAME.hands) do
                if handname ~= context.scoring_name and values.played >= play_more_than and
                    SMODS.is_poker_hand_visible(handname) then
                    reset = false
                    break
                end
            end
...
SMODS.Joker { -- Fanfic Author Twilight
    ...
    loc_vars = function(self, info_queue, card)
        return {
            vars = {card.ability.extra.chips, localize('Pair', 'poker_hands'), card.ability.extra.chips_gain}
        }
    end,
    calculate = function(self, card, context)
        local playedcards = {}
...
SMODS.Joker { -- Queen Chrysalis
    ...
    loc_vars = function(self, info_queue, card)
        return {
            vars = {card.ability.extra.xmult, card.ability.extra.xmult_gain, card.ability.extra.cards_to_destroy}
        }
    end,
    calculate = function(self, card, context)
        if context.setting_blind and context.main_eval and not context.blueprint then
            ...
            for i = 1, card.ability.extra.cards_to_destroy do
                ...
                local card_to_destroy, rem =
                    pseudorandom_element(enhancedcards, pseudoseed("thisdayhasbeenjustperfect"))
...
SMODS.Joker { -- Cozy Glow
    ...
    calculate = function(self, card, context)
        ...
        if context.before and context.main_eval and G.GAME.current_round.hands_played == 0 and #context.full_hand == 1 and
            not context.blueprint and not context.repetition then
            local suits = {}
            local used_suits = {}
            ...
            if cozyglow_card_candidates then
                for i = 1, card.ability.extra.cards_to_destroy do
                    local cozy_destroyed_card, rem = pseudorandom_element(cozyglow_card_candidates,
                        pseudoseed('friendshipispower'))
...
SMODS.Joker { -- Pinkamena
    ...
    loc_vars = function(self, info_queue, card)
        return {
            vars = {card.ability.extra.xmult, card.ability.extra.dollars, card.ability.extra.scoreodds,
                    card.ability.extra.moneyodds, G.GAME.probabilities.normal}
        }
    end,
    calculate = function(self, card, context)
        local ret1 = {
        ...
        if context.individual and context.cardarea == G.play and context.other_card:is_face() then
            ...
            if pseudorandom(pseudoseed('theyreabunchalosers')) < G.GAME.probabilities.normal /
                card.ability.extra.moneyodds then
                table.insert(effects, ret1)
            end
            return SMODS.merge_effects(effects)
        end
...
SMODS.Joker { -- Plainity
    ...
    calculate = function(self, card, context)
        if context.individual and context.cardarea == G.play and SMODS.has_enhancement(context.other_card, "c_base") and
            not context.other_card.edition and not context.other_card.seal then
            return {
                mult = card.ability.extra.mult,
                chips = card.ability.extra.chips
            }
...
SMODS.Joker { -- Flutterbat
    ...
    calculate = function(self, card, context)
        if context.before and context.main_eval and not context.blueprint then
            local enhanced = {}
            for _, scored_card in ipairs(context.scoring_hand) do
                if next(SMODS.get_enhancements(scored_card)) and not scored_card.debuff and
                    not scored_card.MLPflutterbat then
...
SMODS.Joker { -- Babs Seed
    ...
    calculate = function(self, card, context)
        ...
        if context.starting_shop and not context.blueprint then
            local moneygained = math.floor((G.GAME.MLPlastcashout / card.ability.extra.money_req) *
...
SMODS.Joker { -- Autumn Blaze
    ...
    loc_vars = function(self, info_queue, card)
        return {
            vars = {card.ability.extra.repetitions, card.ability.extra.selectedcard}
        }
    end,
    calculate = function(self, card, context)
        ...
        if context.cardarea == G.play and context.repetition and not context.repetition_only and #context.scoring_hand >=
            3 then
        ...
        if context.selling_self then
            G.E_MANAGER:add_event(Event({
                ...
                func = function()
                    ...
                    local poly_card = pseudorandom_element(G.hand.cards,
                        pseudoseed('itsrainbowdashinajarlmaogetitnowlaughatit'))
                    if poly_card then
...
SMODS.Joker { -- Friendship is Benefits
    ...
    loc_vars = function(self, info_queue, card)
        return {
            vars = {card.ability.extra.dollars, card.ability.extra.dollars_gain, localize('Straight', 'poker_hands')}
        }
    end,
    ...
    loc_vars = function(self, info_queue, card)
        ...
        for _, playing_card in pairs(G.playing_cards or {}) do
            ...
            if playing_card.seal then
                seal_tally = seal_tally + 1
            end
            total_tally = card.ability.extra.xmult +
                              (card.ability.extra.xmult_gain * (enhance_tally + edition_tally + seal_tally))
        end
        return {
            vars = {card.ability.extra.xmult, card.ability.extra.xmult_gain, enhance_tally, edition_tally, seal_tally,
                    total_tally}
        }
    end,
    calculate = function(self, card, context)
        ...
        if context.before and not context.blueprint then
            ...
            if not redcheck then
                card.ability.extra.xmult = card.ability.extra.xmult +
                                               (card.ability.extra.xmult_gain * #context.scoring_hand)
                return {
                    message = localize('k_upgrade_ex'),
                    colour = G.C.MULT,
                    card = card
...
SMODS.Joker { -- Mistress Mare-velous
    ...
    calculate = function(self, card, context)
        if context.individual and context.cardarea == G.play and context.other_card and context.other_card:get_id() ==
            11 then
            return {
                xmult = card.ability.extra.xmult
            }
...
SMODS.Joker { -- Hum Drum
    ...
    calculate = function(self, card, context)
        if G.GAME.current_round.hands_left == 1 and not context.blueprint then
            local eval = function()
                return G.GAME.current_round.hands_left <= 0 and not G.RESET_JIGGLES
            end
            juice_card_until(card, eval, true)
        end

        if context.before and G.GAME.current_round.hands_left <= 0 and not context.blueprint and
            card.ability.extra.disableable then
...
SMODS.Joker { -- Mudbriar	
    ...
    calculate = function(self, card, context)
        if context.repetition and context.cardarea == G.play and SMODS.get_enhancements(context.other_card)["m_stone"] ==
            true then
            return {
                message = localize('k_again_ex'),
                repetitions = card.ability.extra.repetitions,
        ...
        elseif context.repetition and context.cardarea == G.hand and
            SMODS.get_enhancements(context.other_card)["m_stone"] == true then
...
SMODS.Joker { -- Flim and Flam
    ...
    loc_vars = function(self, info_queue, card)
        ...
        return {
            vars = {card.ability.extra.mult, card.ability.extra.chips, (card.ability.extra.mult * (voucher_count or 0)),
                    (card.ability.extra.chips * (voucher_count or 0))}
        }
    end,
    calculate = function(self, card, context)
...
SMODS.Joker { -- Flash Sentry
    ...
    loc_vars = function(self, info_queue, card)
        ...
        return {
            vars = {card.ability.extra.dollars, card.ability.extra.dollars * gold_tally}
        }
    end,
    calc_dollar_bonus = function(self, card)
        local gold_tally = 0
...
SMODS.Joker { -- Flurry Heart
    ...
    loc_vars = function(self, info_queue, card)
        return {
            vars = {card.ability.extra.Xmult, card.ability.extra.odds, G.GAME.probabilities.normal}
        }
    end,
    calculate = function(self, card, context)
...
SMODS.Joker { -- Ursa Major
    ...
    loc_vars = function(self, info_queue, card)
        return {
            vars = {card.ability.extra.tarotmake, card.ability.extra.planets}
        }
    end,
    calculate = function(self, card, context)

        if context.using_consumeable and context.consumeable.ability.set == "Planet" and #G.consumeables.cards +
            G.GAME.consumeable_buffer < G.consumeables.config.card_limit and not card.ability.extra.tarotmake then
            card.ability.extra.tarotmake = true
            G.GAME.consumeable_buffer = G.GAME.consumeable_buffer + 1
            G.E_MANAGER:add_event(Event({
...
SMODS.Joker { -- Muffins
    ...
    loc_vars = function(self, info_queue, card)
        return {
            vars = {card.ability.extra.chips, card.ability.extra.chip_loss}
        }
    end,
    rarity = 1,
    atlas = 'MLPJokers3',
            ...
            if card.ability.extra.chips - (card.ability.extra.chip_loss * #G.jokers.cards) <= 0 then
                ...
                return {
                    message = localize {
                        type = 'variable',
                        key = 'a_chips_minus',
                        vars = {card.ability.extra.chip_loss * #G.jokers.cards}
                    },
                    colour = G.C.CHIPS
                }
            end
        ...
        if G.playing_cards then
            for _, playing_card in ipairs(G.playing_cards) do
                if playing_card.ability.played_this_ante then
                    played_tally = played_tally + 1
                end
            end
        end
        return {
            vars = {card.ability.extra.mult, card.ability.extra.mult * played_tally}
        }
    end,
    rarity = 1,
    atlas = 'MLPJokers2',
...
SMODS.Joker { -- Friendship Journal
    key = 'MLPFriendshipJournal',
    config = {
        extra = {
            mult = 0,
            mult_gain = 5,
    ...
    calculate = function(self, card, context)
        if context.setting_blind then
            ...
            for _, poker_hand in ipairs(G.handlist) do
                card.ability.extra.playedhands[poker_hand] = false
                -- print(card.ability.extra.playedhands)
            end
        end
        if context.before and not context.blueprint then
            if not card.ability.extra.playedhands[context.scoring_name] then
                card.ability.extra.playedhands[context.scoring_name] = true
                card.ability.extra.mult = card.ability.extra.mult + card.ability.extra.mult_gain
                -- print(card.ability.extra.playedhands)
                return {
                    message = localize('k_upgrade_ex'),
...
SMODS.Joker { -- The Perfect Pear
    ...
    loc_vars = function(self, info_queue, card)
        return {
            vars = {card.ability.extra.Xmult, localize(card.ability.extra.type, 'poker_hands'),
                    card.ability.extra.cardno}
        }
    end,
    rarity = 3,
    atlas = 'MLPJokers3',
    ...
    calculate = function(self, card, context)
        if context.joker_main and context.scoring_name == card.ability.extra.type and #context.full_hand ==
            card.ability.extra.cardno then
            return {
                xmult = card.ability.extra.Xmult
            }
        end
