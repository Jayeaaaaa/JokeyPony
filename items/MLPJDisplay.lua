...
jd_def["j_MLP_MLPApplejack"] = { -- Applejack
    retrigger_function = function(playing_card, scoring_hand, held_in_hand, joker_card)
        ...
        return ((last_card and playing_card == last_card) or
                   (secondfromlast_card and playing_card == secondfromlast_card)) and
        ...
        if text ~= 'Unknown' then
            for i = 1, #scoring_hand do
                ...
                elseif SMODS.has_any_suit(scoring_hand[i]) then
                    ...
                    table.insert(playedcards,
                        scoring_hand[i].base.id .. scoring_hand[i].base.suit .. scoring_hand[i].ability.name)
                end
                -- print(playedcards)
            end
    ...
    calc_function = function(card)
        ...
        for _, playing_card in ipairs(G.hand.cards) do
            if playing_hand or not playing_card.highlighted then
                if not (playing_card.facing == 'back') and not playing_card.debuff and
                    SMODS.has_enhancement(playing_card, "m_stone") then
...
jd_def["j_MLP_MLPDiscord"] = { -- Discord
    ...
    reminder_text = {{
        ...
        ref_table = "card.joker_display_values",
        ref_value = "MLPDiscord_poker_hand",
        colour = G.C.ORANGE
    }, {
        text = ")"
    }},
        ...
        for _, playing_card in ipairs(G.hand.cards) do
            if playing_hand or not playing_card.highlighted then
                if playing_card.facing and not (playing_card.facing == 'back') and not playing_card.debuff and
                    playing_card:is_suit('Hearts') or playing_card:is_suit('Diamonds') then
                    chips = chips + card.ability.extra.chips *
...
jd_def["j_MLP_MLPLibraryCard"] = { -- Library Card
    ...
    reminder_text = {{
        ...
        ref_table = "card.joker_display_values",
        ref_value = "MLPLibraryCard_poker_hand",
        colour = G.C.ORANGE
    }, {
        text = ")"
    }},
...
jd_def["j_MLP_MLPPinkamena"] = { -- Pinkamena
    ...
    calc_function = function(card)
        ...
        local mnumerator, mdenominator = SMODS.get_probability_vars(card, 1, card.ability.extra.moneyodds,
            'theyreabunchalosers')
        card.joker_display_values.scoreodds = localize {
            type = 'variable',
            key = "jdis_odds",
            vars = {snumerator, sdenominator}
        }
        card.joker_display_values.moneyodds = localize {
            type = 'variable',
            key = "jdis_odds",
            vars = {mnumerator, mdenominator}
        }
    end
}
...
jd_def["j_MLP_MLPAppleJewel"] = { -- Apple Jewel
    ...
    reminder_text = {{
        ...
        ref_table = "card.joker_display_values",
        ref_value = "localized_text_other",
        colour = G.C.ORANGE
    }, {
        text = ")"
    }},
...
jd_def["j_MLP_MLPDaringDo"] = { -- Daring Do
    ...
    calc_function = function(card)
        card.joker_display_values.Xmult = G.GAME.current_round.discards_used == 0 and
                                              (1 + (G.GAME.current_round.discards_left * card.ability.extra.xmult)) or 1
    end
}
jd_def["j_MLP_MLPElements"] = { -- Elements of Harmony
    ...
    calc_function = function(card)
        card.joker_display_values.Xmult =
            (#G.jokers.cards > card.ability.extra.jokers) and (card.ability.extra.xmult) or 1
    end
}
...
jd_def["j_MLP_MLPAppleBloom"] = { -- Apple Bloom
    ...
    calc_function = function(card)
        ...
        if text ~= 'Unknown' then
            for _, scoring_card in pairs(scoring_hand) do
                if #scoring_hand > 1 and scoring_hand[#scoring_hand]:get_id() ==
                    scoring_hand[#scoring_hand - 1]:get_id() then
...
jd_def["j_MLP_MLPMuralCompetition"] = { -- Mural of Competition
    ...
    retrigger_function = function(playing_card, scoring_hand, held_in_hand, joker_card)
        ...
        return (playing_card:get_id() == 6 or playing_card:get_id() == 7 or playing_card:get_id() == 8 or
                   playing_card:get_id() == 9 or playing_card:get_id() == 10) and joker_card.ability.extra.repetitions *
...
jd_def["j_MLP_MLPMuralCompassion"] = { -- Mural of Compassion
    ...
    calc_function = function(card)
        ...
        if text ~= 'Unknown' then
            for _, scoring_card in pairs(scoring_hand) do
                if (scoring_card:get_id() == 6 or scoring_card:get_id() == 7 or scoring_card:get_id() == 8 or
                    scoring_card:get_id() == 9 or scoring_card:get_id() == 10) then
...
jd_def["j_MLP_MLPCadance"] = { -- Princess Cadance
    ...
    calc_function = function(card)
        ...
        for _, playing_card in ipairs(G.hand.cards) do
            if playing_hand or not playing_card.highlighted then
                if not (playing_card.facing == 'back') and not playing_card.debuff and playing_card:is_suit('Hearts') or
                    playing_card:is_suit('Diamonds') then
                    count = count + JokerDisplay.calculate_card_triggers(playing_card, nil, true)
                end
            end
        end
...
jd_def["j_MLP_MLPFriendshipLesson"] = { -- Friendship Lesson
    text = {{
        ref_table = "card.joker_display_values",
        ref_value = "MLPFriendshipLesson_poker_hand",
        colour = G.C.ORANGE
    }},
    text_config = {
        colour = G.C.ORANGE
    ...
    calc_function = function(card)
        card.joker_display_values.MLPFriendshipLesson_poker_hand =
            localize(card.ability.extra.poker_hand, 'poker_hands')
        card.joker_display_values.active = (not card.ability.extra.learned and localize("jdis_active") or
                                               localize("jdis_inactive"))
    end
}
...
jd_def["j_MLP_MLPHumDrum"] = { -- Hum Drum
    ...
    calc_function = function(card)
        local disableable =
            G.GAME and G.GAME.blind and G.GAME.blind.get_type and (G.GAME.blind:get_type() == 'Boss') and
...
jd_def["j_MLP_MLPRichKids"] = { -- Rich Kids
    text = {{
        ref_table = "card.joker_display_values",
        ref_value = "MLPRichKids_poker_hand",
        colour = G.C.ORANGE
    }},
    text_config = {
        colour = G.C.ORANGE
...
jd_def["j_MLP_MLPMudbriar"] = { -- Mudbriar
    retrigger_function = function(playing_card, scoring_hand, held_in_hand, joker_card)
        return (SMODS.get_enhancements(playing_card)["m_stone"]) and joker_card.ability.extra.repetitions *
                   JokerDisplay.calculate_joker_triggers(joker_card) or 0
    end
}
